    ...
    class ParserController extends AbstractController {
        ...
        public function __init(Request $request, FileStorage $storage, ReportParser $parser, ManagerRegistry $doctrine) {
            ...
            if ($request->query->has('file')) {
                $this->file_name = urldecode($request->query->get('file'));

                try {
                    $absolutePath = $storage->resolvePath($this->file_name);
                } catch (\RuntimeException $e) {
                    throw new NotFoundHttpException('File not found!');
                }

                $account_details = $this->get_account_details($doctrine, $this->file_name);
                if (!$account_details) {
